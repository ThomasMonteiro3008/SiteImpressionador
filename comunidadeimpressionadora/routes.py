...
def login():
    ...
    if form_login.validate_on_submit() and 'botao_submit_login' in request.form:
        ...
        if usuario and bcrypt.check_password_hash(senha_cript, form_login.senha.data):
            ...
            par_next = request.arg.get('next')
            if par_next:
                return redirect(par_next)
            else:
                return redirect(url_for('home'))
        else:
            flash(f'Falha no Login, e-mail ou senha incorretas', 'alert-danger')
...
@login_required
def perfil():
    foto_perfil = url_for('static', filename='fotos_perfl/{}'.format(current_user.foto_perfil))
...
def editar_perfil():
    ...
    if form_editar_perfil.validate_on_submit():
        ...
        if form_editar_perfil.foto_perfil.data:
            nome_imagem = salvar_imagem(foto_perfil.data)
